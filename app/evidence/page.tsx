...
export default function EvidencePage() {
  ...
  function getItemPhotoFile(itemId: string) {
    return (
      files.find(
        (f) =>
          f.item_id === itemId && f.file_type === "상품사진" && !!f.file_path,
      ) ?? null
    );
  }
  ...
  function getCostFileForItem(
    itemId: string,
    costType: string,
    wantedFileType: string,
  ) {
  ...
  function getLatestSaleReceiptForItem(itemId: string) {
    ...
    linkedSaleItems.forEach((saleItem) => {
      ...
      const receiptFiles = (
        saleFilesBySaleId.get(saleItem.sale_id) ?? []
      ).filter((f) => f.file_type === SALE_RECEIPT_TYPE && f.file_path);
  ...
  function getLatestSaleForItem(itemId: string) {
    const linkedSaleItems = salesByItem.get(itemId) ?? [];
    if (linkedSaleItems.length === 0) return null;
    const candidates = linkedSaleItems
      .map((s) => saleMap.get(s.sale_id))
  ...
  function getSaleReceiptEntriesForItem(itemId: string): SaleReceiptEntry[] {
    ...
    linkedSaleItems.sort((a, b) => {
      ...
      if (da !== db) return db.localeCompare(da);
      return String(sb?.created_at || "").localeCompare(
        String(sa?.created_at || ""),
      );
    });
  ...
  function getOtherReceiptEntriesForItem(itemId: string): OtherReceiptEntry[] {
    ...
    const matched = costIds
      ...
      .filter(
        (cost): cost is CostRow =>
          !!cost && normalizeCostType(cost.cost_type) === "기타",
      );
    ...
    return matched.map((cost) => {
      const file =
        files
          .filter(
            (f) => f.cost_id === cost.id && f.file_type === "기타비용영수증",
          )
          .sort((a, b) =>
            String(b.created_at || "").localeCompare(
              String(a.created_at || ""),
  ...
  async function saveSaleFile(params: {
    sale_id: string;
    file_type: string;
    file: File;
  }) {
    ...
    const ins = await supabase
      .from("sale_files")
      .insert({ sale_id, file_type, file_path: path });
    if (ins.error) throw ins.error;
  }
  ...
  async function handleSaleReceiptUpload(
    file: File,
    params: { sale_id: string; uploadKey: string },
  ) {
    try {
      setUploadingKey(params.uploadKey);
      setErr(null);
    ...
    try {
      const [
        ...
        supabase
          .from("purchase_items")
          .select(
            "id,purchase_id,item_name,created_at,is_preorder,attachment_url",
          )
        ...
        supabase
          .from("purchase_costs")
          .select(
            "id,purchase_id,cost_type,vendor_name,cost_date,amount,currency",
          ),
        supabase
  ...
  const filteredItems = useMemo(() => {
    ...
    const list = [...searched];
    list.sort((a, b) => {
      const purchaseA = normalizeDate(
        purchaseMap.get(a.purchase_id)?.purchase_date,
      );
      const purchaseB = normalizeDate(
        purchaseMap.get(b.purchase_id)?.purchase_date,
      );
  ...
  const renderDirectFilePreview = (
    ...
    if (!filePath) {
      if (params.canUpload === false)
        return <span style={styles.no}>미업로드</span>;
      return (
      ...
      return (
        <div style={styles.modalPreviewWrap}>
          ...
          <div style={{ ...styles.fileActionCol, gap: 8 }}>
            {params.dateText ? (
              <span style={styles.small}>{params.dateText}</span>
            ) : null}
            {params.countText ? (
              <span style={styles.small}>{params.countText}</span>
            ) : null}
            <div
              style={{
                display: "flex",
    ...
    return (
      <div style={styles.fileCellWrap}>
        <a
          ...
          target="_blank"
          rel="noreferrer"
          style={styles.tinyThumbBox}
        >
          {image ? (
            <img src={fileUrl} alt="첨부파일" style={styles.thumbImg} />
          ) : (
            <span>PDF</span>
          )}
        </a>
    ...
    if (!file?.file_path) {
      ...
      return (
        <label
          style={{
            ...styles.uploadBtn,
            opacity: uploadingKey === params.uploadKey ? 0.6 : 1,
          }}
        >
  ...
  const renderSingleSaleReceiptCell = (
    entry: SaleReceiptEntry | null,
    params: { uploadKey: string },
  ) => {
    ...
    return renderDirectFilePreview(entry.file, {
      ...
      onUpload: async (selected) =>
        handleSaleReceiptUpload(selected, {
          sale_id: entry.sale.id,
          uploadKey: params.uploadKey,
        }),
  ...
  const modalSaleEntries =
    receiptModal?.kind === "sale"
      ? getSaleReceiptEntriesForItem(receiptModal.itemId)
      : [];
  const modalOtherEntries =
    receiptModal?.kind === "other"
      ? getOtherReceiptEntriesForItem(receiptModal.itemId)
      : [];
  ...
  return (
    <div style={styles.page} data-page="evidence">
      <style>{`
        /* PC 표는 그대로 두고, 태블릿 모드에서만 거래처·매입일을 상품 칸으로 합칩니다. */
        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table-wrap'] {
          overflow-x: hidden !important;
        }

        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table'] {
          min-width: 0 !important;
          width: 100% !important;
          table-layout: fixed !important;
        }

        /* 태블릿에서는 사진·거래처·매입일을 상품 칸 안으로 합쳐서 가로 폭을 확보합니다. */
        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-photo'],
        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-supplier'],
        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-date'] {
          display: none !important;
        }

        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-item-meta'] {
          display: block !important;
        }

        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-product-photo'] {
          display: block !important;
          flex: 0 0 auto;
        }

        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table'] th,
        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table'] td {
        ...
        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table'] th:nth-child(2),
        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table'] td:nth-child(2) {
          width: 300px !important;
        }

        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table'] th:nth-child(n+5),
        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table'] td:nth-child(n+5) {
          width: 78px !important;
          text-align: center !important;
        }

        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-product-photo'] img {
          width: 58px !important;
          height: 58px !important;
          object-fit: cover !important;
        }

        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table'] label,
        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table'] button {
          max-width: 100%;
        }

        html[data-view-mode='tablet'] [data-page='evidence'] [data-tablet-role='evidence-table'] [data-tablet-role='evidence-item-meta'] {
          font-size: 11px !important;
        ...
        >
          ...
          <select
            ...
            onChange={(e) =>
              setSort(
                e.target
                  .value as (typeof EVIDENCE_SORT_OPTIONS)[number]["value"],
              )
            }
          >
        ...
        <div style={styles.tableWrap} data-tablet-role="evidence-table-wrap">
          <table style={styles.table} data-tablet-role="evidence-table">
            <thead>
              <tr>
                <th
                  data-tablet-role="evidence-photo"
                  style={{ ...styles.th, width: 110 }}
                >
                  상품사진
                </th>
                <th
                  data-tablet-role="evidence-product"
                  style={{ ...styles.th, width: 180 }}
                >
                  상품
                </th>
                <th
                  data-tablet-role="evidence-supplier"
                  style={{ ...styles.th, width: 140 }}
                >
                  거래처
                </th>
                <th
                  data-tablet-role="evidence-date"
                  style={{ ...styles.th, width: 110 }}
                >
                  매입일
                </th>
            ...
            <tbody>
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((it) => {
                  const purchase = purchaseMap.get(it.purchase_id);
                  const purchaseReceipt = getPurchaseFile(
                    it.purchase_id,
                    "매입영수증",
                  );

                  const supplierShippingCost = getMatchingCostForItem(
                    it.id,
                    "배송비(거래처)",
                  );
                  const forwarderShippingCost = getMatchingCostForItem(
                    it.id,
                    "배송비(배대지)",
                  );
                  ...
                  const supplierShippingReceipt = getCostFileForItem(
                    it.id,
                    "배송비(거래처)",
                    "배송비영수증",
                  );
                  const forwarderShippingReceipt = getCostFileForItem(
                    it.id,
                    "배송비(배대지)",
                    "배송비영수증",
                  );
                  const customsReceipt = getCostFileForItem(
                    it.id,
                    "관부과세",
                    "관부과세영수증",
                  );
                  const balanceReceipt = getCostFileForItem(
                    it.id,
                    "잔금",
                    "잔금비용영수증",
                  );
                  ...
                  return (
                    <tr key={it.id}>
                      ...
                      <td data-tablet-role="evidence-product" style={styles.td}>
                        ...
                        >
                          <div
                            data-tablet-role="evidence-product-photo"
                            style={{ display: "none" }}
                          >
                            {imageUrl ? (
                              <img
                          ...
                          <div style={{ minWidth: 0 }}>
                            ...
                            <div style={styles.small}>
                              매출 {saleEntries.length}건
                            </div>
                            <div
                              data-tablet-role="evidence-item-meta"
                              style={{
                                display: "none",
                                marginTop: 4,
                                fontSize: 12,
                            ...
                            >
                              거래처: {purchase?.supplier ?? "(거래처 없음)"} ·
                              매입일: {fmtDate(purchase?.purchase_date)}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td
                        ...
                        style={styles.td}
                      >
                        {purchase?.supplier ?? "(거래처 없음)"}
                      </td>
                      <td data-tablet-role="evidence-date" style={styles.td}>
                      ...
                      <div key={entry.sale.id} style={styles.modalItemCard}>
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div style={{ ...styles.small, fontSize: 14 }}>
                            구분:{" "}
                            {entry.sale.channel ||
                              entry.sale.sales_channel ||
                              "미입력"}
                          </div>
                          <div style={{ ...styles.small, fontSize: 14 }}>
                            판매금액: {fmtKRW(entry.saleItem.line_total)}
                          </div>
                          <div style={{ ...styles.small, fontSize: 14 }}>
                            수량: {entry.saleItem.qty ?? 0}
                          </div>
                        </div>
                        {entry.file?.file_path ? (
                          renderDirectFilePreview(entry.file, {
                            ...
                            onDelete: async () =>
                              deleteSaleFileRow(entry.file!, uploadKey),
                          })
                        ) : (
                          <label
                            style={{
                              ...styles.uploadBtn,
                              opacity: uploadingKey === uploadKey ? 0.6 : 1,
                            }}
                          >
                  ...
                  return (
                    <div key={entry.cost.id} style={styles.modalItemCard}>
                      <div style={{ minWidth: 0 }}>
                        ...
                        >
                          날짜: {fmtDate(entry.cost.cost_date)}
                        </div>
                        <div style={{ ...styles.small, fontSize: 14 }}>
                          거래처: {entry.cost.vendor_name || "미입력"}
                        </div>
                        <div style={{ ...styles.small, fontSize: 14 }}>
                          금액: {fmtKRW(entry.cost.amount)}{" "}
                          {entry.cost.currency
                            ? `(${entry.cost.currency})`
                            : ""}
                        </div>
                      ...
                      {entry.file?.file_path ? (
                        renderDirectFilePreview(entry.file, {
                          ...
                          onDelete: async () =>
                            deletePurchaseFileRow(entry.file!, uploadKey),
                        })
                      ) : (
                        <label
                          style={{
